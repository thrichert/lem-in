...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/06 12:36:09 by apopinea          #+#    #+#             */
/*   Updated: 2017/04/06 12:36:11 by apopinea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_cv_dd_2(t_ft_printf_env *e, long long int nb)
{
	ft_di_up_config(e);
	ft_setting_di(e, nb);
...
{
	if (!(e->config & B_EXCLU))
		ft_cv_dd_2(e, (long int)va_arg(ap, long int));
	else if (e->config & B_J)
		ft_cv_dd_2(e, (intmax_t)va_arg(ap, intmax_t));
	else if (e->config & B_LL || e->config & B_L)
		ft_cv_dd_2(e, (long long int)va_arg(ap, long long int));
	else
		ft_cv_dd_2(e, (long int)va_arg(ap, long int));
}
