...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/06 12:43:04 by apopinea          #+#    #+#             */
/*   Updated: 2017/04/06 12:43:06 by apopinea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_cv_xx_2(t_ft_printf_env *e, unsigned long long int nb)
{
	ft_x_up_config(e, nb);
	ft_setting_x(e, nb);
	ft_print_x(e);
...
**			un type different
**		ft_cv_xx_2 -> ft_printf/ft_cv_xx.c
...
void			ft_cv_xx(t_ft_printf_env *e, va_list ap)
{
	e->signe = 'X';
	if (!(e->config & B_EXCLU))
		ft_cv_xx_2(e, (unsigned int)va_arg(ap, unsigned int));
	else if (e->config & B_J)
		ft_cv_xx_2(e, (uintmax_t)va_arg(ap, uintmax_t));
	else if (e->config & B_LL)
		ft_cv_xx_2(e,
			(long long unsigned int)va_arg(ap, long long unsigned int));
	else if ((e->config & B_L) || (e->config & B_Z))
		ft_cv_xx_2(e, (long unsigned int)va_arg(ap, long unsigned int));
	else if (e->config & B_BL)
		ft_cv_xx_2(e, (unsigned int)va_arg(ap, unsigned int));
	else if (e->config & B_H)
		ft_cv_xx_2(e, (unsigned short)va_arg(ap, unsigned int));
	else
		ft_cv_xx_2(e, (unsigned char)va_arg(ap, unsigned int));
}
